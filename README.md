# tumor_classification_engine v0.0.1

## Overview

Small tumor classification engine for tests propose only
...
```
mkvirtualenv marvin_tumor_classification_engine_env
```
...
The default log level is set to _WARNING_. You can change the log level at runtime setting another value to one of the following environment variable: `MARVIN_TUMOR_CLASSIFICATION_ENGINE_LOG_LEVEL` or `LOG_LEVEL`. The available values are _CRITICAL_, _ERROR_, _WARNING_, _INFO_ and _DEBUG_.
